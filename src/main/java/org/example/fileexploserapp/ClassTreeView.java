...
public class ClassTreeView extends FileExplorerFx {
    ...
    public TreeItem<String>[] TreeCreate(File dir){
        ...
        for(int i=0; i<fl.length; i++){
            ...
            {A[pos] =new TreeItem<>(fl[i].getName(), new ImageView(new Image(Objects.requireNonNull(ClassLoader.getSystemResourceAsStream("img/folderOpen.png"))))); pos++;}
            else if(!fl[i].isFile()&& !fl[i].isHidden() && fl[i].isDirectory() && n>0){
                A[pos] = new TreeItem<>(fl[i].getName(), new ImageView(new Image(Objects.requireNonNull(ClassLoader.getSystemResourceAsStream("img/folderOpen.png")))));
                try{
    ...
    public void CreateTreeView(TreeView<String> treeview){
        ...
        TreeItem<String> ThisPc = new TreeItem<>("This PC", new ImageView(new Image(Objects.requireNonNull(ClassLoader.getSystemResourceAsStream("img/pc.png")))));
        TreeItem<String>[] drives = new TreeItem[sysroots.length];
        for(int i=0; i<sysroots.length;i++){
            drives[i] = new TreeItem<>(sysroots[i].getAbsolutePath(), new ImageView(new Image(Objects.requireNonNull(ClassLoader.getSystemResourceAsStream("img/thumb_Hard_Drive.png")))));
            try{
